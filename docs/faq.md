---
description: >-
  Frequently asked questions about TooGoodToGo Auto: configuration, functionality,
  rate limits and more.
---

# FAQ

## Is TooGoodToGo Auto a bot?

Yes. TooGoodToGo Auto is a free, open-source bot for Too Good To Go (TGTG): it
...
store). TooGoodToGo Auto works with any item you can see in the app.

## How can I get Magic Bags before they sell out?

Popular magic bags are often gone within seconds. Add the items you want and
TooGoodToGo Auto monitors them for you, sends a notification the moment one is
...
## Can I configure TooGoodToGo Auto to only notify me when a surprise bag is available?

Yes. TooGoodToGo Auto can send push notifications through the free
...
Yes. TooGoodToGo Auto is completely free and open-source (MIT licensed). You only
...
| OS | Path |
| --- | --- |
| **macOS** | `~/Library/Caches/TooGoodToGo-Auto/` |
| **Linux** | `~/.cache/TooGoodToGo-Auto/` (or `$XDG_CACHE_HOME/TooGoodToGo-Auto/`) |
| **Windows** | `%LOCALAPPDATA%\TooGoodToGo-Auto\Cache\` |
...
in again. Add the `TooGoodToGo-Auto/` folder to the tool's exclude list to prevent
this.
