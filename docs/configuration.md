...
| `NTFY_TOPIC` | string | Optional topic name for [Ntfy.sh](https://ntfy.sh) push notifications. Leave it empty to monitor in the terminal without notifications. A topic is required when automatic checkout is enabled so 3DS prompts can be delivered. Subscribe to the same topic in the ntfy app to receive alerts.<br>You can find the Ntfy setup guide [here](https://docs.ntfy.sh).<br><br>**Note:** Make sure you pick a unique string to prevent other users receiving your notifications. This could be a random [UUID](https://www.uuidgenerator.net/) or a random [password](https://1password.com/password-generator). |
